...
function singleManualUploader(option){//url,picIdCont
	...
	manualUploader = new qq.FineUploader({
        ...
        callbacks:{
            ...
            onCancel: function(){
                var imgList=$("#"+option.element+" .qq-uploader-selector .qq-upload-list-selector li")
                if(imgList.length<=1){
            ...
            onComplete: function (id, fileName, responseJSON, maybeXhr) {
                ...
                if(option.element == "fine-uploader-manual-trigger-license" || option.element =="fine-uploader-manual-trigger-aptitude"){
                	$("#"+option.element + " .qq-upload-list-selector .qq-upload-success").hide()
                	console.log(option.element)
                	//重新调用一次刷新图片的方法
                	console.log(option.functionName)
                	option.functionName()
                }
            },
        }
